...
def event_analytics(request, view='summary'):
    ...
    if request.GET.get('dt_from', False):
        ...
        dt_from = dt_from.replace(tzinfo=pytz.timezone(user_tz))
        arrow_from = arrow.get(dt_from).isoformat()
    if request.GET.get('dt_to', False):
        try:
            ...
            return HttpResponseBadRequest(err_msg)
        dt_to = dt_to.replace(tzinfo=pytz.timezone(user_tz))
        arrow_to = arrow.get(dt_to).isoformat()
    ...
    if request.GET.get('value', False):
        ...
        # Show Resources
            ...
            resource_events = events.get_report(key='resource-title', value=resource_name, timespan=timespan,
                                                request=request)
        ...
        # Show Types
            ...
            type_events = events.get_report(key='resource-type', value=type_name, timespan=timespan, request=request)
